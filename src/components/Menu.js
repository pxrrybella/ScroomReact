...
import { projectUserweb } from "../services/ProjectService";

const Menu = ({ userweb }) => {
    const [projects, setProjects] = useState([]);
    const userwebUsername = userweb.user_name;

    const listaProyectos = async() => {
        setProjects(await projectUserweb(userwebUsername));
    }
    ...
    return (
        <div className="menu col-sm-3">
            <div className="mt-2 mb-5 d-flex flex-column align-items-center">
                ...
                <h4 class="mb-3 text-center">{projects[0]}</h4>
            ...
            <div className="d-flex flex-column col-m-4 align-items-center">
                {projects.map(project => (
                    <button type="button" class="text-sm-start btn btn-dark btn-lg m-1">{project}</button>
                ))}
...
const MenuAnalytics = ({ userweb }) => {
    const userwebUsername = userweb.user_name
    console.log("USERWEBUSERNAME***", userwebUsername);
    const [projects, setProjects] = useState([]);

    useEffect(() => {
        const getProjects = async() => {
            setProjects(await projectUserweb(userwebUsername));
            console.log("**GET PROJECTS**",await projectUserweb(userwebUsername));
        }
        getProjects();
